...
def save_detection_json(
    ...
    output_path = os.path.join(
        output_dir,
        f"{detection_data['camera_id']}.json"
    )
    ...
    try:
        ...
        print(
            f"[JSON ERROR] {detection_data['camera_id']}: {e}",
            flush=True
        )
...
class RoundRobinNPUWorker:
    ...
    def _loop(self):
        ...
        while self.running:
            ...
            # Cooperative scheduling point: keep capture/GUI responsive while
            # the NPU worker continues doing the same detection work.
            time.sleep(0)
...
def build_live_grid_mosaic(
    sources,
    target_w=1280,
    target_h=720
):
    """
    Build the display from the newest camera frame only.

    The current stream frame is resized directly to its grid cell.
    The latest completed NPU detections are then drawn on that cell.
    Therefore the video can stay live while the boxes lag behind.
    """
    ...
    for index, source in enumerate(sources):
        ...
        x2 = (
            target_w
            if col == cols - 1
            else min(target_w, x1 + cell_w)
        )

        y2 = (
            target_h
            if row == rows - 1
            else min(target_h, y1 + cell_h)
        )

        dst_w = x2 - x1
        dst_h = y2 - y1

        # IMPORTANT: no full-resolution copy here.
        ok, frame, frame_id = source.get_latest_frame(
            copy=False
        )
        ...
        if not ok or frame is None:
            ...
            mosaic[y1:y2, x1:x2] = cell
            continue

        # The displayed video is always the newest captured frame.
        cell = cv.resize(
            frame,
            (dst_w, dst_h),
            interpolation=cv.INTER_NEAREST
        ...
        # Latest completed detection. This is intentionally allowed to lag
        # behind the current video frame.
        (
            boxes,
            scores,
            classes,
        ...
        if orig_w > 0 and orig_h > 0:
            ...
            sx = dst_w / float(orig_w)
            sy = dst_h / float(orig_h)
        ...
        cv.putText(
            cell,
            f"{source.name} | Persons: {person_count}",
            (
                6,
                min(banner_h - 6, 20)
            ),
    ...
    try:
        ...
        while True:
            ...
            if show_gui and now >= next_display:
                ...
                # ==================================================================
                # LIVE VIDEO / DELAYED BOXES
                # ==================================================================
                # Always display the newest captured frame. The latest completed
                # NPU boxes are drawn on top, so only the boxes can lag.

                mosaic = build_live_grid_mosaic(
                    sources,
                    args.display_width,
                    args.display_height
                )
                ...
                cv.putText(
                    ...
                    (
                        f"Total NPU FPS: "
                        f"{worker.total_inference_fps:.2f} | "
                        f"Total Persons: "
                        f"{total_people}"
                    ),
                    (
            ...
            if (
                ...
                print(
                    (
                        ...
                        f"CPU={cpu:.1f}% | "
                        f"TOTAL CAPTURE FPS="
                        f"{total_capture_fps:.1f} | "
                        f"TOTAL NPU FPS="
                        f"{worker.total_inference_fps:.2f} | "
                        f"NPU="
                        f"{detector.inference_ms:.1f}ms"
                    ),
                    flush=True
                )
