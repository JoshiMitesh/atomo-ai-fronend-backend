...
async function generateEventsWorkbook() {
  ...
  sheet.columns = [
    { header: 'Date', key: 'date', width: 14 }, { header: 'Time', key: 'time', width: 14 },
    { header: 'Name', key: 'name', width: 24 }, { header: 'Status', key: 'status', width: 18 },
    { header: 'Camera', key: 'camera', width: 24 }, { header: 'Confidence', key: 'confidence', width: 14 },
    { header: 'Photo', key: 'photo', width: 20 }
  ];
  const header = sheet.getRow(1); header.font = { bold: true }; header.alignment = { vertical: 'middle', horizontal: 'center' }; header.height = 24;

  // Hard maximum: newest 5000 events only.
  ...
  for (const event of events) {
    ...
    const row = sheet.addRow({
      ...
      camera: event.camera_name || 'Manual Upload', confidence: Number(event.score || 0)
    });
    row.height = 82; row.alignment = { vertical: 'middle' }; row.getCell('confidence').numFmt = '0%';
    if (event.crop_filename) {
      const cropPath = path.join(db.CROPS_DIR, path.basename(event.crop_filename));
      if (fs.existsSync(cropPath)) try {
        const imageId = workbook.addImage({ filename: cropPath, extension: path.extname(cropPath).toLowerCase() === '.png' ? 'png' : 'jpeg' });
        sheet.addImage(imageId, { tl: { col: 6.12, row: row.number - 0.88 }, ext: { width: 88, height: 88 }, editAs: 'oneCell' });
      } catch (err) { console.warn(`[Report] Could not embed ${event.crop_filename}: ${err.message}`); }
    }
  }
...
function reportMiddleware(req,res,next) {
  ...
  generateEventsWorkbook().then(async workbook => {
    ...
    res.setHeader('Content-Disposition',`attachment; filename="face-recognition-report-${stamp}.xlsx"`);
    await workbook.xlsx.write(res); res.end();
  }).catch(err => { console.error('[Report] Failed:',err); if(!res.headersSent) res.status(500).json({error:'Failed to generate Excel report.'}); else res.end(); });
}

express.application.use = function(...args) {
  if (!reportMiddlewareInstalled) { reportMiddlewareInstalled=true; originalUse.call(this,reportMiddleware); }
...
console.log('[Report] Excel event reporting enabled (maximum 5000 events).');
